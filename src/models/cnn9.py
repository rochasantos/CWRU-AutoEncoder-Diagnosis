...
# The model uses Layer Normalization after each convolutional layer
class CNN1D(nn.Module):
    def __init__(self, input_length, num_classes=2):
        ...
        self.norm1 = nn.LayerNorm([16, input_length - 16 + 1])  # normaliza canais

        self.conv2 = nn.Conv1d(in_channels=16, out_channels=32, kernel_size=16)
        self.pool2 = nn.MaxPool1d(kernel_size=8)
        self.norm2 = nn.LayerNorm([32, (input_length - 16 + 1) // 8 - 16 + 1])  # normaliza canais

        self.conv3 = nn.Conv1d(in_channels=32, out_channels=64, kernel_size=16)
        self.pool3 = nn.MaxPool1d(kernel_size=16)
        self.norm3 = nn.LayerNorm([64, ((input_length - 16 + 1) // 8 - 16 + 1) // 8 - 16 + 1])  # normaliza canais

        # Dummy input to calculate sizes after conv/pool for LayerNorm
