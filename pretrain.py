...
from src.models import CNN14, BinaryCNN
from src.data_processing import VibrationMapBuilder, VibrationDatasetFromMap


def pre_train(lr, num_epochs, device='cuda'):

    model = BinaryCNN(input_length=9600)
    print(f"Model: {model}")

    # uored
    train_map, val_map = VibrationMapBuilder("data/processed/uored/bi").get_split(val_ratio=0.3)
    train_dataset = VibrationDatasetFromMap(train_map, transform=None)
    val_dataset = VibrationDatasetFromMap(val_map, transform=None)

    # hust
    # train_map_h, val_map_h = VibrationMapBuilder("data/processed/hust/bi").get_split(val_ratio=0.1)
    ...
    val_loader=val_loader = DataLoader(val_dataset, batch_size=32)
    
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    criterion = torch.nn.CrossEntropyLoss()
    
    checkpoint_path = f"checkpoint/binary_uored.pth"
