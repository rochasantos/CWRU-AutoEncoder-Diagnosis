...
def make_objective(all_train_data, n_splits=4, seed=42, task_type="CPU"):
    """
    Para cada trial:
      - monta um CatBoost com os params propostos
      - calcula a acurácia média de CV (SGKFold) em CADA setup
      - retorna a média das acurácias entre os setups (maximize)
    """
    ...
    def objective(trial):
        ...
        for pack in all_train_data:
            ...
            model = CatBoostClassifier(
                **params,
                loss_function="MultiClass",  # definir só aqui
            )

            # Em CV não passamos eval_set (sem early stopping) para manter API do sklearn
...
# -------------- main --------------
def main(task_type="CPU", n_trials=80):
    ...
    for pack_train, pack_test in zip(all_train_data, all_test_data):
        ...
        # Para o treinamento final, podemos usar early stopping no próprio test set
        # (se preferir, separe um hold-out; aqui seguimos padrão simples)
        final.fit(
            X_tr, y_tr,
            eval_set=(X_te, y_te),
            use_best_model=True,
...
if __name__ == "__main__":
    # CPU por padrão. Se quiser GPU e Poisson, chame main(task_type="GPU")
